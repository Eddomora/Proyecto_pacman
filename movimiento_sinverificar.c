...
int mov_Fanta(int largo, int ancho, int **tablero, struct posicion *fant, struct posicion *Pma ,int num_ente ){
    ...
    case 0: //Movimiento hacia Arriba
        if (tablero[((fant->y)-1+largo)%largo][fant->x]==2){
            return 0;
    ...
    case 2: //Movimiento hacia la Derecha
        if (tablero[fant->y][((fant->x)+1+ancho)%ancho]==2){
            return 0;
...
int mov_Pacman(int largo, int ancho ,int **tablero ,int move ,struct posicion *pacm, struct fantasmas *enemy){
    ...
    switch (move){
        case 'a': //Movimiento hacia la Izquierda
            if (tablero[pacm->y][((pacm->x)-1+ancho)%ancho]==3 || tablero[pacm->y][((pacm->x)-1+ancho)%ancho]==4 || tablero[pacm->y][((pacm->x)-1+ancho)%ancho]==5 || tablero[pacm->y][((pacm->x)-1+ancho)%ancho]==6){
                return 0;
        ...
        case 'w': //Movimiento hacia Arriba
            if (tablero[((pacm->y)-1+largo)%largo][pacm->x]==3 || tablero[((pacm->y)-1+largo)%largo][pacm->x]==4 || tablero[((pacm->y)-1+largo)%largo][pacm->x]==5 || tablero[((pacm->y)-1+largo)%largo][pacm->x]==6){
                    return 0;
